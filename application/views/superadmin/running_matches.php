                            ...
                            <tbody>
                                <?php $i = 1; foreach($crickets as $c):?>
                                  <tr>
                                    ...
                                    <td><a href="<?=base_url('SuperAdmin/matchOdds?match_id='.$c['event_id'].'&market_id='.$c['market_id']);?>" data-toggle="tooltip" title="View Match Odds">Match Odds</a>&nbsp;&nbsp;<a href="<?=base_url('SuperAdmin/viewMatchFancy?match_id='.$c['event_id'].'&market_id='.$c['market_id']);?>" data-toggle="tooltip" title="View Fancy list">Fancy List</a></td>
                                    <td>
                                      <?=anchor("SuperAdmin/playPauseMatch?id=".$c['id'],$c['match_result'],array('onclick' => "return confirm('Do you want to change this match status')"))?>
                                    </td>
                                    <td><input type="text" name="cricbuzz_id" id="cricbuzz_id" value="<?=$c['cricbuzz_id'];?>" onkeyup="saveCricbuzzId(this.value, '<?=$c['id'];?>')"></td>
                                  </tr>
                                <?php endforeach;?>
                            </tbody>
                        </table>
...
<script>
    ...
    function resultDeclare()
    {
      swal({   
          ...
          $('.loader').show();
          $.ajax({
            url : "<?php echo site_url('SuperAdmin/resultDeclare')?>",
            type: "POST",
    ...
    function saveCricbuzzId(cricbuzz_id, id) {
      $.ajax({
          url : "<?php echo site_url('SuperAdmin/saveCricbuzzId')?>",
          type: "GET",
