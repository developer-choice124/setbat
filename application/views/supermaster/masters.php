        ...
        <div class="col-sm-12">
            <div class="panel panel-info">
                ...
                <div class="panel-body">
                    <div id="infoMessage"><?php if ($this->session->flashdata('message')) {
                                                echo $this->session->flashdata('message');
                                            } ?></div>
                    <form action="<?= base_url('SuperMaster/addUser'); ?>" method="post" class="form-horizontal">
                            ...
                            <tbody>
                                ...
                                foreach ($users as $user) : ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="<?= base_url('SuperMaster/users?master_id=' . $user->id); ?>">View Users</a>
                                        </td>
                                        <td><?= $user->commission > 0 ? $user->commission . '%' : 'NONE'; ?></td>
                                        <td><input type="number" min="0" max="3" class="form-control form-control-sm" id="<?= $user->id ?>-session-commission" value="<?= $user->session_commission; ?>" onchange="setSessionCommission(this.value,'<?= $user->id; ?>')" /></td>
                                        <td><input type="number" min="0" max="2" class="form-control form-control-sm" id="<?= $user->id ?>-odd-commission" value="<?= $user->odd_commission; ?>" onchange="setOddCommission(this.value,'<?= $user->id; ?>')" /></td>
                                        <td>
                                            <?php if ($id == $user->id) {
                                                echo $user->active == 0 ? 'Block' : 'Active';
                                            } else {
                                                echo $user->active == 0 ? '<a href="' . base_url('SuperMaster/activateUser?id=' . $user->id . '&status=' . $user->active) . '">Block</a>' : '<a href="' . base_url('SuperMaster/activateUser?id=' . $user->id . '&status=' . $user->active) . '">Active</a>';
                                            } ?>
                                        </td>
                                        <td>
                                            ...
                                            <a href="<?= base_url('SuperMaster/editUser?user_id=' . $user->id) ?>"><i class="ti-pencil-alt" data-toggle="tooltip" title="Edit"></i></a>&nbsp;&nbsp;
                                            ...
                                            <a href="<?= base_url('SuperMaster/userBetHistory?user_id=' . $user->id) ?>" data-toggle="tooltip" title="Bet History"><span class="label label-info rounded-0" style="border-radius: 0;">B</span></a>
                                            <a href="<?= base_url('SuperMaster/userAccountStatement?user_id=' . $user->id) ?>" data-toggle="tooltip" title="Account Statement"><span class="label label-info rounded-0" style="border-radius: 0;">S</span></a>
                                            <a href="<?= base_url('SuperMaster/userProfitLoss?user_id=' . $user->id) ?>" data-toggle="tooltip" title="Profit & Loss"><span class="label label-info rounded-0" style="border-radius: 0;">P-L</span></a>
                                            <a href="javascript:void(0)" onclick="blockMatches('<?= $user->id?>', 'blockMatchesbutton<?= $user->id?>')" id="blockMatchesbutton<?= $user->id?>" data-user-event='<?= json_encode($user->blocked_event); ?>' title="Block Match"><span class="label label-success rounded-0" style="border-radius: 0;">B-M</span></a>
                                            <?php if ($user->show_match == 'yes') { ?>
                                                <a href="#" onclick="showMatch('<?= $user->id ?>','no')" data-toggle="tooltip" title="Match is Showing"><span class="label label-success rounded-0" style="border-radius: 0;">M</span></a>&nbsp;&nbsp;
                                            <?php } else { ?>
                                                <a href="#" onclick="showMatch('<?= $user->id ?>','yes')" data-toggle="tooltip" title="Match is Paused"><span class="label label-danger rounded-0" style="border-radius: 0;">M</span></a>&nbsp;&nbsp;
                                            <?php } ?>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
...
<div id="childModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            ...
            <form action="<?= base_url('SuperMaster/addChild'); ?>" method="POST">
...
<div id="passwordModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            ...
            <form action="<?= base_url('SuperMaster/resetUserPassword'); ?>" method="POST">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
...
<div id="paymentModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            ...
            <form action="<?= base_url('SuperMaster/addMoney'); ?>" method="POST">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
            ...
            <hr />
            <form method="post" action="<?= base_url('SuperMaster/witdrawChips'); ?>">
                <div class="modal-body">
...
<div id="userModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            ...
            <form action="<?= base_url('SuperMaster/updateUserInfo'); ?>" method="POST">
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                    ...
                    <div class="row">
                        <div class="form-group">
                            ...
                            <div class="col-md-4">
                                <label for="description">Bet Delay(Fancy)</label>
...
<script>
    ...
    function removeUser(id) {
        swal({
            ...
            $.ajax({
                url: "<?php echo site_url('SuperMaster/deleteUser') ?>/" + id,
                type: "POST",
                //dataType: "JSON",
    ...
    function maxChips(chips) {
        var max = '<?= $chips->balanced_chips ? $chips->balanced_chips : 0; ?>';
        max = parseInt(max);
        if (chips >= max) {
    ...
    function userInfo(id) {
        $.ajax({
            url: "<?php echo site_url('SuperMaster/userInfo?user_id=') ?>" + id,
            type: "POST",
            //dataType: "JSON",
            success: function(data) {
                console.log(data);
    ...
    function lockBetting(id, status) {
        swal({
            ...
            $.ajax({
                url: "<?php echo site_url('SuperMaster/lockBetting?user_id=') ?>" + id + "&status=" + status,
                type: "POST",
                //dataType: "JSON",
                success: function(data) {
                    location.reload();
    ...
    function showMatch(id, status) {
        swal({
            title: "Are you sure?",
            text: "You want to Play/Pause Matches for this user!",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            confirmButtonText: "Yes, Do it!",
            ...
            $.ajax({
                url: "<?php echo site_url('SuperMaster/showMatch?user_id=') ?>" + id + "&status=" + status,
                type: "POST",
                //dataType: "JSON",
                success: function(data) {
                    location.reload();
    ...
    function setSessionCommission(commission, id) {
        if (parseFloat(commission) > 3) {
            swal({
                title: "Error",
                text: "Maximum allowed percentage for odd market is 3",
                type: "error"
            })
            $("#" + id + "-session-commission").val(3);
        } else {
            $.ajax({
                url: "<?php echo site_url('SuperMaster/updateSessionCommission?user_id=') ?>" + id + "&commission=" + commission,
                type: "POST",
                //dataType: "JSON",
                success: function(data) {
                    console.log(data)
    ...
    function setOddCommission(commission, id) {
        if (parseFloat(commission) > 2) {
            swal({
                title: "Error",
                text: "Maximum allowed percentage for odd market is 2",
                type: "error"
            })
            $("#" + id + "-odd-commission").val(2);
        } else {
            $.ajax({
                url: "<?php echo site_url('SuperMaster/updateOddCommission?user_id=') ?>" + id + "&commission=" + commission,
                type: "POST",
                //dataType: "JSON",
                success: function(data) {
                    console.log(data)
