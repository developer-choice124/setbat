                        ...
                        <table id="" class="table table-bordered table-striped" cellspacing="0" width="100%">
                            ...
                            <tbody>
                              <?php $i = 1; foreach ($matches as $mkey => $m) { ?>
                                <tr>
                                  <td><?=$i++;?></td>
                                  <td><a href="<?=base_url('Admin/matchOdds?market_id='.$m['market_id'].'&match_id='.$m['event_id']);?>"><?=$m['event_name'];?></a></td>
                                  <td><!-- <a href="<?=base_url('Admin/matchOdds?market_id='.$m['market_id'].'&match_id='.$m['event_id']);?>">
                                  ...
                                  <td>
                                    <!-- <?php foreach($m['odds'][0]['teams'] as $r):?>
                                      <a href="<?=base_url('Admin/matchOdds?market_id='.$m['market_id'].'&match_id='.$m['event_id']);?>" class="btn btn-darkblue" style="color: white;">
                                        <?=$r->back['price'];?>
                                      </a>
                                      <a href="<?=base_url('Admin/matchOdds?market_id='.$m['market_id'].'&match_id='.$m['event_id']);?>" class="btn btn-danger" style="color: white;">
                                        <?=$r->lay['price'];?>
                                      </a>
                                    <?php endforeach;?> -->
                                  </td>
