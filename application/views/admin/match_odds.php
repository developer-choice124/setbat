    ...
    <div class="col-sm-12">
      <div class="panel panel-info">
        <div class="panel-heading">
          ...
          <div class="pull-right"><a href="<?= base_url('Admin/runningCricket'); ?>">Back</a><a href="#" data-perform="panel-collapse"><i class="ti-minus"></i></a></div>
        </div>
                ...
                <div class="table-responsive" id="singleMatchTable">
                  <table class="table table-bordered table-condensed" width="100%">
                    <tr>
                      ...
                      <th style="background: #008efa; color: white; border: none !important; min-width: 50px;  max-width: 50px;">
                        <center>back</center>
                      </th>
                      <th style="background: red; color: white; border: none !important; min-width: 50px; max-width: 50px;">
                        <center>lay</center>
                      </th>
                    </tr>
                    ...
                    foreach ($matchOdds as $mk => $mo) { ?>
                      <tr>
                        ...
                        <td style="background: #b5e0ff; cursor: pointer;">
                          <center><b><?= $mo->back['price']; ?></b><br /><?= $mo->back['size']; ?></center>
                        </td>
                        <td style="background: #ffbfcd; cursor: pointer;">
                          <center><b><?= $mo->lay['price']; ?></b><br /><?= $mo->lay['size']; ?></center>
                        </td>
                      </tr>
                    <?php } ?>
                  </table>
...
<script type="text/javascript">
  ...
  function scoreReload() {
    $.ajax({
      url: "<?php echo site_url('Admin/scoreReload?market_id=') ?>" + marketId + "&match_id=" + matchId,
      type: "GET",
      dataType: 'json',
  ...
  function profitNLoss() {
    $.ajax({
      url: "<?php echo site_url('Admin/profitNLoss?market_id=') ?>" + marketId,
      type: "POST",
      dataType: 'json',
  ...
  function panaReload() {
    var mid = "<?= $match->market_id; ?>";
    $.ajax({
      url: "<?php echo site_url('Admin/showPana?market_id=') ?>" + mid,
      type: "POST",
      success: function(data) {
        //console.log(data);
        $("#panaTable").html(data);
