...
class Master extends MY_Controller {
    ...
    public function allUsers() {
        ...
        $hdata['p_l'] = $this->p_l;
        $hdata['heading'] = $this->panel->title;
        $data['users'] = $this->_outlist($this->Common_model->get_data_by_query("select * from users_with_groups where group_name = 'user' and parent_id = $this->id and deleted = 'no' order by id DESC"));
    ...
    public function runningCricket() {
        ...
        $this->load->view('layout/backend_header', $hdata);
        $today = date('Y-m-d');
        $data['crickets'] = $this->Common_model->get_data_by_query("SELECT * FROM running_matches WHERE match_result = 'running'");
        $this->load->view('layout/backend_sidebar');
    ...
    public function profitNLoss() {
        ...
        foreach ($runners as $rk => $r) {
            $tid = $r['selectionId'];
            $allTeams[$rk]['back'] = $this->Common_model->get_single_query("SELECT SUM(profit) AS p, SUM(loss) AS l FROM bet WHERE user_id IN(SELECT id FROM users WHERE parent_id = $this->id) AND market_id = '$market_id' AND team_id = $tid AND back_lay = 'back' AND bet_type = 'matched'");
            $allTeams[$rk]['lay'] = $this->Common_model->get_single_query("SELECT SUM(profit) AS p, SUM(loss) AS l FROM bet WHERE user_id IN(SELECT id FROM users WHERE parent_id = $this->id) AND market_id = '$market_id' AND team_id = $tid AND back_lay = 'lay' AND bet_type = 'matched'");
            $allTeams[$rk]['tid'] = $tid;
        }
        ...
        foreach ($runners as $rk => $r) {
            $tid = $r['selectionId'];
            $res[$rk]['pl'] = ($masterPartnership*$this->calculateResult($allTeams,$rk))/100;
            $res[$rk]['id'] = $tid;   
        }
    ...
    public function getBookedFancy() {
        ...
        $mid = $this->input->get('market_id');
        $bets = $this->Common_model->get_data_by_query("SELECT a.* FROM bet a LEFT JOIN users_with_groups b ON a.user_id = b.id WHERE a.user_id IN(SELECT id FROM users WHERE parent_id = $this->id) AND a.market_id = '$mid' AND a.team_id = $fid AND a.bet_type = 'fancy'");
        ...
        foreach ($odds as $ok => $o) {
            $b = $this->Common_model->get_single_query("SELECT SUM(a.profit) AS p, SUM(a.loss) AS l FROM bet a LEFT JOIN users_with_groups b ON a.user_id = b.id WHERE a.user_id IN(SELECT id FROM users WHERE parent_id = $this->id) AND a.market_id = '$mid' AND a.team_id = $fid AND a.bet_type = 'fancy' AND a.odd = ".$o['odd']." AND back_lay = 'back'");
            $l = $this->Common_model->get_single_query("SELECT SUM(a.profit) AS p, SUM(a.loss) AS l FROM bet a LEFT JOIN users_with_groups b ON a.user_id = b.id WHERE a.user_id IN(SELECT id FROM users WHERE parent_id = $this->id) AND a.market_id = '$mid' AND a.team_id = $fid AND a.bet_type = 'fancy' AND a.odd = ".$o['odd']." AND back_lay = 'lay'");
            echo '<tr>
                    <td>'.$i++.'</td>
                    <td>'.$o['name'].'</td>
