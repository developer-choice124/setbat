...
class SuperMaster extends MY_Controller {
    ...
    public function masters() {
        ...
        $hdata['p_l'] = $this->p_l;
        $hdata['heading'] = $this->panel->title;
        $data['users'] = $this->_outlist($this->Common_model->get_data_by_query("select * from users_with_groups where group_name = 'master' and parent_id = $this->id and deleted = 'no' order by id DESC"));
    ...
    public function users() {
        ...
        if(isset($mid)) {
            ...
            $data['users'] = $this->_outlist($this->Common_model->get_data_by_query("SELECT * FROM `users_with_groups` WHERE group_name = 'user' AND parent_id IN (SELECT id FROM users WHERE parent_id = $this->id) AND deleted = 'no' ORDER BY id DESC"));
        }
        $data['groups'] = $this->Common_model->get_data_by_query("select * from groups where name = 'master'");
        $data['chips'] = $this->chips;
        $this->load->view('layout/backend_header', $hdata);
    ...
    public function profitNLoss() {
        ...
        foreach ($masters as $mk => $mv) {
            ...
            foreach ($runners as $rk => $rv) {
                $abcd[$rk]['pl'] += ($supermasterPartnership*$this->calculateResult($allTeams,$rk))/100;
            }
        }
        $res = array();
        foreach ($runners as $rk => $r) {
    ...
    public function getBookedFancy() {
        ...
        $mid = $this->input->get('market_id');
        $bets = $this->Common_model->get_data_by_query("SELECT a.* FROM bet a LEFT JOIN users_with_groups b ON a.user_id = b.id WHERE a.user_id IN (SELECT id FROM users WHERE parent_id IN (SELECT id FROM users WHERE parent_id = $this->id)) AND a.market_id = '$mid' AND a.team_id = $fid AND a.bet_type = 'fancy'");
