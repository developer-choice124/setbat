...
class User extends MY_Controller {
    ...
    public function userBalanceDuringPlaceBetOdds($mkid) {
    ...
    public function profitNLossDuringPlaceBetMatched($mid) {
        $ateams = $this->Common_model->findfield('running_matches','market_id',$mid,'teams');
        $teams = json_decode($ateams);
        $team1 = $teams[0]->id;
        $team2 = $teams[1]->id;
        ...
        $data = array('t1' => $team1win, 't2' => $team2win);
        return $data;
    }
    ...
    public function placeBet() {
        if($this->lockBetting == 'yes') {
            ...
            if($btype == 'matched') {
                $pl = $this->profitNLossDuringPlaceBetMatched($mkid);
                $teams = json_decode($ateams);
                $team1 = $teams[0]->id;
                $team2 = $teams[1]->id;
                if($back_lay == 'back') {
                    if($team1 == $this->input->post('team_id')) {
                        $t1pl = $pl['t1'] + $profit;
                        $t2pl = $pl['t2'] - $loss;
                        $t1loss = $t1pl >= 0 ? 0 : $t1pl;
                        $t2loss = $t2pl >= 0 ? 0 : $t2pl;
                    } else {
                        $t1pl = $pl['t1'] - $loss;
                        $t2pl = $pl['t2'] + $profit;
                        $t1loss = $t1pl >= 0 ? 0 : $t1pl;
                ...
                if($t1loss < 0 && $t2loss < 0) {
                  $finalLoss = $t1loss > $t2loss ? $t1loss : $t2loss;
                }
                ...
                $maxLimit = $finalLoss;
                $maxUse = $chips->current_chips - $maxLimit;
                if ($maxUse > $chips->balanced_chips) {
                    $place = 'no';
                }
            }
            if($btype == 'unmatched') {
