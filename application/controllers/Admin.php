...
class Admin extends MY_Controller
{
    ...
    public function supermasters()
    {
        ...
        $hdata['heading'] = $this->panel->title;
        $hdata['p_l'] = $this->p_l;
        $users = $this->_outlist($this->Common_model->get_data_by_query("select * from users_with_groups where group_name = 'supermaster' and deleted = 'no' order by id DESC"));
    ...
    public function masters()
    {
        ...
        if (isset($smid)) {
            ...
            $users = $this->_outlist($this->Common_model->get_data_by_query("select * from users_with_groups where group_name = 'master' and deleted = 'no' order by id DESC"));
        }
        foreach ($users as $key => $user) {
            $users[$key]->blocked_event = $this->Common_model->get_data_by_query("SELECT *  FROM `event_block` where user_id = $user->id order by id asc");
        }
    ...
    public function users()
    {
        ...
        if (isset($mid)) {
            ...
            $users = $this->_outlist($this->Common_model->get_data_by_query("SELECT * FROM `users_with_groups` WHERE group_name = 'user' AND deleted = 'no' ORDER BY id DESC"));
        }
        foreach ($users as $key => $user) {
            $users[$key]->blocked_event = $this->Common_model->get_data_by_query("SELECT *  FROM `event_block` where user_id = $user->id order by id asc");
        }
