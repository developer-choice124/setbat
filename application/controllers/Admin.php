...
class Admin extends MY_Controller {
	...
	public function supermasters() {
		...
		$hdata['heading'] = $this->panel->title;
		$hdata['p_l'] = $this->p_l;
		$users = $this->_outlist($this->Common_model->get_data_by_query("select * from users_with_groups where group_name = 'supermaster' and deleted = 'no' and parent_id = $this->id order by id DESC"));
		...
		$this->load->view('layout/backend_footer', $fdata);
	}
	...
	public function masters() {
		...
		if (isset($smid)) {
			$users = $this->_outlist($this->Common_model->get_data_by_query("select * from users_with_groups where group_name = 'master' and parent_id = $smid and deleted = 'no' order by id DESC"));
		} else {
			$users = $this->_outlist($this->Common_model->get_data_by_query("SELECT * FROM users_with_groups WHERE group_name = 'master' AND parent_id IN (SELECT id FROM users_with_groups WHERE parent_id = $this->id ) and deleted = 'no' order by id DESC"));
		}
		foreach ($users as $key => $user) {
			$users[$key]->blocked_event = $this->Common_model->get_data_by_query("SELECT *  FROM `event_block` where user_id = $user->id order by id asc");
		}
	...
	public function users() {
		...
		if (isset($mid)) {
			...
			$users = $this->_outlist($this->Common_model->get_data_by_query("SELECT * FROM `users_with_groups` WHERE group_name = 'user' AND parent_id IN (SELECT id FROM users_with_groups WHERE parent_id IN (SELECT id FROM users_with_groups WHERE parent_id = $this->id )) AND deleted = 'no' ORDER BY id DESC"));
		}
		foreach ($users as $key => $user) {
			$users[$key]->blocked_event = $this->Common_model->get_data_by_query("SELECT *  FROM `event_block` where user_id = $user->id order by id asc");
		}
	...
	public function getBookedFancy() {
		...
		$mid = $this->input->get('market_id');
		$bets = $this->Common_model->get_data_by_query("SELECT a.*, b.parent_id FROM bet a LEFT JOIN users_with_groups b ON a.user_id = b.id WHERE a.market_id = '$mid' AND a.team = '$runner' AND a.bet_type = 'fancy' AND b.id IN (SELECT id FROM users WHERE parent_id IN (SELECT id FROM users WHERE parent_id IN (SELECT id FROM users WHERE parent_id = $this->id))) ORDER BY a.odd ASC");
		echo '<h4>' . $runner . '</h4>';
		...
		foreach ($odds as $ok => $o) {
			$final = 0;
			foreach ($o as $uuid) {
				$yy = $this->Common_model->get_single_query("SELECT SUM(a.profit) AS p FROM bet a WHERE a.market_id = '$mid' AND a.team = '$runner' AND a.bet_type = 'fancy' AND a.odd = $ok AND a.back_lay = 'back' AND a.user_id = $uuid");
				$nn = $this->Common_model->get_single_query("SELECT SUM(a.profit) AS p FROM bet a WHERE a.market_id = '$mid' AND a.team = '$runner' AND a.bet_type = 'fancy' AND a.odd = $ok AND a.back_lay = 'lay' AND a.user_id = $uuid");
				$pp = isset($yy) ? $yy->p : 0;
				$ll = isset($nn) ? $nn->p : 0;
				...
				$smc = $supermaster->commission; //supermaster partnershipt to admin
				if (empty($smc)) {
					$smc = 0;
				}

				$calc = $pp - $ll;
				$smsCommission = $calc - abs($calc) * ($smc) / 100;
				$final += $smsCommission;
			}
			if ($final > 0) {
				...
				if ($user->odd_commission > 0 || $master->odd_commission > 0) {
					...
					if ($userLoss < 0) {
						...
						$mcommissiondcdata = array(
							'txnid' => md5(microtime()),
							'user_id' => $mid,
							'credits' => ($masterCommission * ($smc + $mc) / 100),
							'credited_from' => $smid,
							'debits' => $userCommission,
							'debited_to' => $uid,
							'balance' => $mbal + ($masterCommission * ($smc + $mc) / 100) - $userCommission,
							'assigned_by' => $smid,
							'transaction_date' => date('Y-m-d H:i:s'),
							'description' => "Commission from " . $match->event_name,
							'type' => 'bet',
							'commission' => 'yes',
							'match_id' => $match_id,
							'set_bal' => ($masterCommission * ($smc + $mc) / 100) - $userCommission,
							'parent_bal' => ($masterCommission * ($smc + $mc) / 100),
							'updated_at' => date('Y-m-d H:i:s'),
						);
						...
						$smcommissiondcdata = array(
							'txnid' => md5(microtime()),
							'user_id' => $smid,
							'credits' => $masterCommission * $smc / 100,
							'credited_from' => $aid,
							'debits' => ($masterCommission * ($smc + $mc) / 100),
							'debited_to' => $mid,
							'balance' => $smbal + ($masterCommission * $smc / 100) - ($masterCommission * ($smc + $mc) / 100),
							'assigned_by' => $aid,
							'transaction_date' => date('Y-m-d H:i:s'),
							'description' => "Commission from " . $match->event_name,
							'type' => 'bet',
							'commission' => 'yes',
							'match_id' => $match_id,
							'set_bal' => ($masterCommission * $smc / 100) - ($masterCommission * ($smc + $mc) / 100),
							'parent_bal' => $masterCommission * $smc / 100,
							'updated_at' => date('Y-m-d H:i:s'),
						);
						...
						$acommissiondcdata = array(
							'txnid' => md5(microtime()),
							'user_id' => $aid,
							'debits' => $masterCommission * $smc / 100,
							'debited_to' => $smid,
							'balance' => $abal - ($masterCommission * $smc / 100),
							'assigned_by' => 8,
							'transaction_date' => date('Y-m-d H:i:s'),
							'description' => "Commission from " . $match->event_name,
							'type' => 'bet',
							'commission' => 'yes',
							'match_id' => $match_id,
							'set_bal' => -($masterCommission * $smc / 100),
							'parent_bal' => $masterCommission * $smc / 100,
							'updated_at' => date('Y-m-d H:i:s'),
						);
