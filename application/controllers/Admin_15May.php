...
class Admin extends MY_Controller
{
    ...
    public function deleteUserBet()
    {
        ...
        redirect('Admin/userBetHistory?user_id=' . $uid);
    }
    ...
    public function betHistory()
    {
        ...
        //pagination
        $config = array();
        $config["base_url"] = base_url() . "Admin/betHistory";
    ...
    public function fancyReload()
    {
        ...
        $fancy = $this->match->matchFancies($match->market_id);
                ...
                if ($user->odd_commission > 0 || $master->odd_commission > 0) {
                    ...
                    if ($userLoss < 0) {
                        ...
                        $mcommissiondcdata = array(
                            'txnid'             => md5(microtime()),
                            'user_id'           => $mid,
                            'credits'           => ($masterCommission* ($smc + $mc) / 100) + ($userCommission * ($smc + $mc) / 100),
                            'credited_from'     => $smid,
                            'debits'            => $userCommission,
                            'debited_to'        => $uid,
                            'balance'           => $mbal + ($masterCommission* ($smc + $mc) / 100) + $userCommission * ($smc + $mc) / 100 - $userCommission,
                            'assigned_by'       => $smid,
                            'transaction_date'  => date('Y-m-d H:i:s'),
                            'description'       => "Commission from " . $match->event_name,
                            'type'              => 'bet',
                            'commission'        => 'yes',
                            'match_id'          => $match_id,
                            'set_bal'           => ($masterCommission* ($smc + $mc) / 100) + $userCommission * ($smc + $mc) / 100 - $userCommission,
                            'parent_bal'        => ($masterCommission* ($smc + $mc) / 100) + $userCommission * ($smc + $mc) / 100,
                            'updated_at'        => date('Y-m-d H:i:s')
                        );
                        ...
                        $smcommissiondcdata = array(
                            'txnid'             => md5(microtime()),
                            'user_id'           => $smid,
                            'credits'           => $userCommission * ($smc) / 100 + $masterCommission * $smc / 100,
                            'credited_from'     => $aid,
                            'debits'            => ($masterCommission* ($smc + $mc) / 100) + $userCommission * ($smc + $mc) / 100,
                            'debited_to'        => $mid,
                            'balance'           => $smbal + ($userCommission * ($smc) / 100 + $masterCommission * $smc / 100) - (($masterCommission* ($smc + $mc) / 100) + $userCommission * ($smc + $mc) / 100),
                            'assigned_by'       => $aid,
                            'transaction_date'  => date('Y-m-d H:i:s'),
                            'description'       => "Commission from " . $match->event_name,
                            'type'              => 'bet',
                            'commission'        => 'yes',
                            'match_id'          => $match_id,
                            'set_bal'           => ($userCommission * ($smc) / 100 + $masterCommission * $smc / 100)  - (($masterCommission* ($smc + $mc) / 100) + $userCommission * ($smc + $mc) / 100),
                            'parent_bal'        => $userCommission * ($smc) / 100 + $masterCommission * $smc / 100,
                            'updated_at'        => date('Y-m-d H:i:s')
                        );
                        ...
                        $acommissiondcdata = array(
                            'txnid'             => md5(microtime()),
                            'user_id'           => $aid,
                            'debits'            => $userCommission * ($smc) / 100 + $masterCommission * $smc / 100,
                            'debited_to'        => $smid,
                            'balance'           => $abal - ($userCommission * ($smc) / 100 + $masterCommission * $smc / 100),
                            'assigned_by'       => 8,
                            'transaction_date'  => date('Y-m-d H:i:s'),
                            'description'       => "Commission from " . $match->event_name,
                            'type'              => 'bet',
                            'commission'        => 'yes',
                            'match_id'          => $match_id,
                            'set_bal'           => - ($userCommission * ($smc) / 100 + $masterCommission * $smc / 100),
                            'parent_bal'        => $userCommission * ($smc) / 100 + $masterCommission * $smc / 100,
                            'updated_at'        => date('Y-m-d H:i:s')
                        );
    ...
    public function profitLossFancyCalculation($bid, $mid, $result)
    {
        ...
        $this->Crud_model->insert_record('credits_debits', $udcdata);
        // user commission data
        if ($user->session_commission > 0) {
            $ucommissiondcdata = array(
                'txnid'             => md5(microtime()),
                'user_id'           => $bet->user_id,
                'credits'           => $bet->user_commission,
                'credited_from'     => $mid,
                'balance'           => $ufinalBal + $bet->user_commission,
                'assigned_by'       => $mid,
                'transaction_date'  => $bet->created_at,
                ...
                'bet_id'            => $bet->id,
                'match_id'          => $bet->match_id,
                'set_bal'           => $bet->user_commission,
                'parent_bal'        => $bet->user_commission,
                'updated_at'        => date('Y-m-d H:i:s')
            );
            $this->Crud_model->insert_record('credits_debits', $ucommissiondcdata);
        }
        ...
        if ($master->session_commission > 0 || $user->session_commission > 0) {
            $mcommissiondcdata = array(
                'txnid'             => md5(microtime()),
                'user_id'           => $mid,
                'credits'           => $bet->master_commission*($smc + $mc)/100 + $bet->user_commission * ($smc + $mc) / 100,
                'credited_from'     => $smid,
                'debits'            => $bet->user_commission,
                'debited_to'        => $uid,
                'balance'           => $mfinalBal + $bet->master_commission*($smc + $mc)/100 - $bet->user_commission + $bet->user_commission * ($smc + $mc) / 100,
                'assigned_by'       => $smid,
                'transaction_date'  => $bet->created_at,
                'description'       => "Commission from " . $bet->match_name,
                'type'              => 'bet',
                ...
                'bet_id'            => $bet->id,
                'match_id'          => $bet->match_id,
                'set_bal'           => $bet->master_commission*($smc + $mc)/100 - $bet->user_commission + $bet->user_commission * ($smc + $mc) / 100,
                'parent_bal'        => ($bet->master_commission*($smc + $mc)/100 + $bet->user_commission * ($smc + $mc) / 100),
                'updated_at'        => date('Y-m-d H:i:s')
            );
            $this->Crud_model->insert_record('credits_debits', $mcommissiondcdata);
        }
        ...
        if ($master->session_commission > 0 || $user->session_commission > 0) {
            $smcommissiondcdata = array(
                'txnid'             => md5(microtime()),
                'user_id'           => $smid,
                'credits'           => $bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100,
                'credited_from'     => $aid,
                'debits'            => $bet->master_commission*($smc + $mc)/100 + $bet->user_commission * ($smc + $mc) / 100,
                'debited_to'        => $mid,
                'balance'           => $smfinalbal + ($bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100) - ($bet->master_commission*($smc + $mc)/100 + $bet->user_commission * ($smc + $mc) / 100),
                'assigned_by'       => $aid,
                'transaction_date'  => $bet->created_at,
                'description'       => "Commission from " . $bet->match_name,
                'type'              => 'bet',
                ...
                'bet_id'            => $bet->id,
                'match_id'          => $bet->match_id,
                'set_bal'           => ($bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100)  - ($bet->master_commission*($smc + $mc)/100 + $bet->user_commission * ($smc + $mc) / 100),
                'parent_bal'        => ($bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100),
                'updated_at'        => date('Y-m-d H:i:s')
            );
            $this->Crud_model->insert_record('credits_debits', $smcommissiondcdata);
        }
        ...
        if ($master->session_commission > 0 || $user->session_commission > 0) {
            $acommissiondcdata = array(
                'txnid'             => md5(microtime()),
                'user_id'           => $aid,
                'debits'            => $bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100,
                'debited_to'        => $smid,
                'balance'           => $afinalbal - ($bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100),
                'assigned_by'       => 8,
                'transaction_date'  => $bet->created_at,
                'description'       => "Commission from " . $bet->match_name,
                'type'              => 'bet',
                ...
                'bet_id'            => $bet->id,
                'match_id'          => $bet->match_id,
                'set_bal'           => - ($bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100),
                'parent_bal'        => ($bet->user_commission * ($smc) / 100 + $bet->master_commission * $smc / 100),
                'updated_at'        => date('Y-m-d H:i:s')
            );
            $this->Crud_model->insert_record('credits_debits', $acommissiondcdata);
    ...
    {
        $users = $this->Common_model->get_data_by_query("SELECT id FROM users_with_groups");
        foreach ($users as $u) {
            $uid = $u['id'];
            $cdb = $this->Common_model->get_single_query("SELECT * FROM credit_debit_balance WHERE user_id = $uid");
            $cd = $this->Common_model->get_single_query("SELECT SUM(credits) AS c, SUM(debits) AS d FROM credits_debits WHERE user_id = $uid");
            $credit = $cd->c;
            $debit = $cd->d;
            ...
            if(isset($cdb)) {
                ...
                $this->Crud_model->edit_record('credit_debit_balance', $cdb->id, $data);
            } else {
                $credit = $cd->c;
                $debit = $cd->d;
                $balance = $credit - $debit;
                ...
                $this->Crud_model->insert_record('credit_debit_balance', $data);
            }
            $this->db->where('user_id',$uid); 
            $this->db->set('balance_calculated', 'yes');
            $this->db->update('credits_debits');
        }
    }
    ...
    public function panelTitle()
    {
        ...
        $this->load->view('layout/backend_sidebar');
        $this->load->view('admin/panel_title', $data);
        $this->load->view('layout/backend_footer');
    }
