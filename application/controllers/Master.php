...
class Master extends MY_Controller
{
    ...
    public function getBookedFancy()
    {
        ...
        $mkid = $this->input->get('market_id');
        $bets = $this->Common_model->get_data_by_query("SELECT a.* FROM bet a LEFT JOIN users_with_groups b ON a.user_id = b.id WHERE a.market_id = '$mkid' AND a.team = '$runner' AND a.bet_type = 'fancy' AND a.user_id IN (SELECT id FROM users WHERE parent_id = $this->id) ORDER BY a.odd ASC");
        echo '<h4>' . $runner . '</h4>';
        ...
        if (empty($smc)) $smc = 0;
        $aid = $supermaster->parent_id; //admin id
        $admin = $this->Common_model->get_single_query("select * from users where id = $aid");
        $ac = $admin->commission;
        if (empty($ac)) $ac = 0;
        $msCommission = 100 - ($ac + $smc + $mc);
        foreach ($odds as $ok => $o) {
            $yy = $this->Common_model->get_single_query("SELECT SUM(a.profit) AS p FROM bet a WHERE a.market_id = '$mkid' AND a.team = '$runner' AND a.bet_type = 'fancy' AND a.odd = $o AND a.back_lay = 'back'");
            $nn = $this->Common_model->get_single_query("SELECT SUM(a.profit) AS p FROM bet a WHERE a.market_id = '$mkid' AND a.team = '$runner' AND a.bet_type = 'fancy' AND a.odd = $o AND a.back_lay = 'lay'");
            $pp = isset($yy) ? $yy->p : 0;
            $ll = isset($nn) ? $nn->p : 0;
            $final = 0;
            $final = $pp - $ll;
            $msFinal = abs($final) - abs($final) * $msCommission / 100;
            if ($final > 0) {
                $show = '<span class="text-danger">' . $msFinal . '</span>';
            } else {
                $show = '<span class="text-success">' . $msFinal . '</span>';
            }
            $aa .= '<tr>
                    <td>' . $o . '</td>
                    <td>' . $show . '</td>
