...
class User extends MY_Controller {
    ...
    public function showMatches(){
        ...
        $getUSerDetail->parent_detail->parent_detail = $this->Common_model->get_record_by_id("users" ,$sid);
        $aid = $getUSerDetail->parent_detail->parent_detail->parent_id;
        
        $EventList = $this->Common_model->get_data_by_query("SELECT * FROM event_block WHERE user_id IN ($user_id, $getUSerDetail->parent_id, $sid, $aid )");
        $eventIds = [];
    ...
    public function placeBet() {
        if ($this->lockBetting == 'yes') {
            ...
            $done = 'yes';
            if($btype == 'fancy') {
                $actualLoss = $this->checkFancyGaliOnPlaceBet($mkid,$team,$odd,$type);
                if($actualLoss < $loss) {
                    $done = 'no';
                }
            }
    ...
    public function checkFancyGali() {
        ...
        if(is_numeric($odd)) {
            if ($type == 'back') {
                $fbl = $this->Common_model->get_single_query("SELECT SUM(loss) as l FROM bet WHERE user_id = $this->id AND market_id = '$mkid' AND back_lay = '$type' AND bet_type = 'fancy' AND team = '$team' AND odd >= '$odd'");
            } else {
                $fbl = $this->Common_model->get_single_query("SELECT SUM(loss) as l FROM bet WHERE user_id = $this->id AND market_id = '$mkid' AND back_lay = '$type' AND bet_type = 'fancy' AND team = '$team' AND odd <= '$odd'");
            }
            $loss = $fbl->l ? $fbl->l : 0;
